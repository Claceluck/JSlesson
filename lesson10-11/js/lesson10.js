...
class PrivateProp {
    #privateProp = 100; // приватное свойство (доступно только внутри класса)
    _privateProp = 120; // условно приватное свойство ( доступо везде, но _ указывает , что доаступ должен осуществляться через get / set )
...
// obj.#privateProp = 500;
// console.log(#privateProp); //  SyntaxError: Private 
...
class UnitHelper {
    // static метод принадлежит не объекту а классу, для выззова нет не обходимости создовать объект 
    //  static  обычно исполььззуеться для создания фабричных или утилитарных методов (например math.floor())
    static getUnit (unitType, unitName){
        if (unitType.toLowerCase() === "knight") return new Knight(unitName); // knight
...
let userInput2 = prompt("введите тип Юнита");

let userUnit1 = UnitHelper.getUnit(userInput1, userInput1);
let userUnit2 = UnitHelper.getUnit(userInput2, userInput2);
